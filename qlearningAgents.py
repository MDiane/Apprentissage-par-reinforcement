...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        bestAction = [ac for ac in self.getLegalActions(state) \
                                              if self.getQValue(state, ac) == bestValue]
    ...
    def update(self, state, action, nextState, reward):
        ...
        newVal = (1 - a) * qVal + a * (reward + d * nextVal)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        for f in feat:
          self.weights[f] += self.alpha * corr * feat[f]
